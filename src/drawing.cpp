...
void setup_lighting(const Shader &lights_shader,
                    const Shader &lights_box_shader,
  ...
  auto pointLightPositions =
      std::array{glm::vec3(state.building_position.x + 8.5f,
                           state.building_position.y + 3.2f,
                           state.building_position.z + 4.5f),
                 glm::vec3(state.building_position.x + 6.5f,
                           state.building_position.y + 3.7f,
                           state.building_position.z - 3.0f),
                 glm::vec3(state.building_position.x - 5.2f,
                           state.building_position.y + 3.7f,
                           state.building_position.z + 9.0f),
                 glm::vec3(state.building_position.x - 10.5f,
                           state.building_position.y + 3.7f,
                           state.building_position.z + 7.0f),
                 glm::vec3(state.building_position.x + 0.5f,
                           state.building_position.y + 6.0f,
                           state.building_position.z + 3.0f)};
  ...
  constexpr static auto point_light_colors =
      std::array{glm::vec3(5.0f, 5.0f, 5.0f), glm::vec3(10.0f, 0.0f, 0.0f),
                 glm::vec3(0.0f, 0.0f, 15.0f), glm::vec3(0.0f, 5.0f, 0.0f)};
  ...
  lights_shader.setFloat("spotLight.outerCutOff",
...
void draw_building(const Model &building_model, const Shader &shader,
                   const ProgramState &state) {
  glm::mat4 model = glm::mat4(1.0f);
  model = glm::translate(model,
                         state.building_position); // translate it down so it's
                                                   // at the center of the scene
  model = glm::scale(
      model, glm::vec3(state.building_scale)); // it's a bit too big for our
...
void draw_instances(const Instances &instances, const Shader &shader,
                    const ProgramState &state) {
  const auto &frame_memo = state.frame_memo;
  // set bender shader
  shader.use();
