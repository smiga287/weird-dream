...
Instances::Instances(const Model &instance_model, const Shader &shader,
                     int size)
    : bender_model_(instance_model), SIZE(size),
      models_(generate_random_models(size)), shader_(shader) {
  initOpenGL();
}

Instances::~Instances() { deinitOpenGL(); }
...
void Instances::initOpenGL() {
  ...
  glBufferData(GL_ARRAY_BUFFER, SIZE * sizeof(glm::mat4), &models_[0],
               GL_STATIC_DRAW);
...
void Instances::render() const {
  ...
  glBindTexture(GL_TEXTURE_2D,
                bender_model_.textures_loaded[0]
