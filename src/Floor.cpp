...
Floor::Floor(const Shader &shader) : shader_(shader) { initOpenGL(); }
...
void Floor::initOpenGL() {
  ...
  glBufferData(GL_ARRAY_BUFFER, floor_vertices_.size() * sizeof(float),
               floor_vertices_.data(), GL_STATIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, floor_EBO_);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, floor_indices_.size() * sizeof(uint),
               floor_indices_.data(), GL_STATIC_DRAW);
...
void Floor::loadTextures() {
  floor_specular_map_ = loadTexture(
      FileSystem::getPath("resources/textures/floor_diffuse_map.jpg"));
  floor_diffuse_map_ = loadTexture(
      FileSystem::getPath("resources/textures/floor_specular_map.jpg"));
...
void Floor::render(const glm::mat4 &model, const glm::mat4 &view,
