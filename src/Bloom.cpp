...
void Bloom::computeGaussianBlur() {
    ...
    for (uint i = 0; i < amount; i++) {
        ...
        glBindTexture(GL_TEXTURE_2D, first_iteration ? colorBuffers_[1] : pingpongColorbuffers_[!horizontal_]);  // bind texture of other framebuffer (or scene if first iteration)
        quad_.render();
...
Bloom::Bloom(const Shader &lights_shader, const Shader &lights_box_shader,
             ...
             : lights_shader_(lights_shader), lights_box_shader_(lights_box_shader),
               lights_blur_shader_(lights_blur_shader), lights_merger_shader_(lights_merger_shader) {
    initOpenGL();
}
